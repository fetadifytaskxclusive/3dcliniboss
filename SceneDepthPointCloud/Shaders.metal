...
vertex void unprojectVertex(uint vertexID [[vertex_id]],
    ...
    // Aplicar filtro de distância - ignorar pontos fora do alcance
    if (depth < uniforms.minScanDistance || depth > uniforms.maxScanDistance) {
        return;
    }
    
    // Ler os dados de confiança (High = 2, Medium = 1, Low = 0)
    // Usamos read() em vez de sample() porque é uma textura unsigned int e previne a interpolação linear (que cria pontos fantasmas)
    uint2 confCoord = uint2(texCoord.x * confidenceTexture.get_width(), texCoord.y * confidenceTexture.get_height());
    const auto confidence = float(confidenceTexture.read(confCoord).r);
    
    // Apenas permitir pontos com confiança MAIOR OU IGUAL ao que o usuário escolheu na tela!
    // (Ex: se ele escolheu Low/0, todos passam. Se escolheu High/2, só High passa).
    if (confidence < float(uniforms.confidenceThreshold)) {
        return; // Descarta imediatamente pontos abixos da qualidade exigida
    }
...
fragment float4 particleFragment(ParticleVertexOut in [[stage_in]],
    ...
    // Nós renderizamos o "splat" (ponto) inteiro como um quadrado sólido.
    // O descarte em círculo criava falhas visuais enormes (espaços pretos) entre os pontos.
    if (in.color.a == 0) {
        discard_fragment();
    }
