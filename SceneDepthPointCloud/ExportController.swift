...
class ExportController : UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    private let viewIn3DButton = UIButton(type: .system)
    ...
    override func viewDidLoad() {
        ...
        goToSaveCurrentViewButton.setTitle("Ir para Scan Atual", for: .normal)
        ...
        exportLabel.text = "Scans Salvos: \(exportData.count) encontrados"
        exportLabel.font = .systemFont(ofSize: 18, weight: .bold)
        ...
        exportButton.setTitle("Exportar Scan Selecionado", for: .normal)
        ...
        viewIn3DButton.tintColor = .systemBlue
        viewIn3DButton.setTitle("Visualizar em 3D", for: .normal)
        viewIn3DButton.setImage(.init(systemName: "cube.transparent"), for: .normal)
        viewIn3DButton.translatesAutoresizingMaskIntoConstraints = false
        viewIn3DButton.addTarget(self, action: #selector(viewIn3D), for: .touchUpInside)
        view.addSubview(viewIn3DButton)
        
        deleteButton.tintColor = .red
        deleteButton.setTitle("Excluir Scan Selecionado", for: .normal)
        ...
        NSLayoutConstraint.activate([
            ...
            viewIn3DButton.widthAnchor.constraint(equalToConstant: 250),
            viewIn3DButton.heightAnchor.constraint(equalToConstant: 50),
            viewIn3DButton.bottomAnchor.constraint(equalTo: deleteButton.topAnchor, constant: -12),
            viewIn3DButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
    ...
    @objc func viewIn3D() {
        guard let fileURL = selectedExport else { return }
        
        let viewer = PointCloudViewer3D()
